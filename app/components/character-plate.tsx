...
export function CharacterPlate({ character, options }: Props) {
  return (
    <div
      style={{
      ...
      className={cn(
        'relative flex w-full min-w-0 flex-row items-center justify-between overflow-hidden rounded-md border-2 p-2 pl-20 pr-4 transition-all sm:pl-24',
        options?.isChecked
          ? 'border-primary opacity-100 shadow-md grayscale-0'
          : 'contrast-90 border-transparent opacity-50 grayscale hover:border-muted-foreground/40 hover:opacity-90 hover:grayscale-0',
      )}
    >
